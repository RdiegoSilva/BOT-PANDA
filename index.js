...
case 'bneon':
		...
		case 'hargapremium':
					...
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/558881060472',MessageType.text, { quoted: mek} )
					break
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Adesivo Responder / Marcar!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*É O DIABO* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
