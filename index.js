...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Diego\n' //ganti nama lu! 
            + 'ORG: Panda;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=558881060472:+55 8881060472\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '#'
blocked = []   
limitawal = 999999999
...
/******** OWNER NUMBER**********/
const ownerNumber = ["558881060472@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["558881060472@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('O código QR está pronto, digitalize agora ..','white'),color('DIEGO SILVAツ','red'),color('X','lime'),color('BOT PANDA','red'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Olá* @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nnão se esqueça da introdução\nSe Register Por Favor! @${num.split('@')[0]} 🗣`
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 2) {
            ...
            role = 'Grau particular 1 ⚌¹'
        } else if (levelRole <= 14) {
            role = 'Grau particular 2 ⚌²'
        } else if (levelRole <= 16) {
            role = 'Grau particular 3 ⚌³'
        } else if (levelRole <= 18) {
            role = 'Grau particular 4 ⚌⁴'
        } else if (levelRole <= 20) {
            role = 'Grau particular 5 ⚌⁵'
            ...
            role = 'Sargento Grau 1 ≣¹'
        } else if (levelRole <= 34) {
            role = 'Sargento Grau 2 ≣²'
        } else if (levelRole <= 36) {
            role = 'Sargento Grau 3 ≣³'
        } else if (levelRole <= 38) {
            role = 'Sargento Grau 4 ≣⁴'
        } else if (levelRole <= 40) {
            role = 'Sargento Grau 5 ≣⁵'
           ...
           var prema = 'Free'
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("CUIDADO COM A FALA 😠")
                        .then(() => client.groupRemove(from, sender))
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\nBOT-PANDA vai chutar você por falar asperamente!`, text ,{quoted: mek})
                        }).catch(() => client.sendMessage(from, `Felizmente, não sou um administrador, Acho Que o administrador já o expulsou!`, text , {quoted : mek}))
                } else {
                    return reply( "Por favor, cuide do discurso de Min 😇")
                }
            }
        }
		...
		if (isGroupAdmins) return reply('já que você é um administrador de grupo, os bots não irão chutá-lo')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#permissão admin")) return reply("#permissão aceita")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link de grupo detectado, desculpe ${sender.split("@")[0]} Você será expulso do grupo`)
		setTimeout( () => {
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('558881060472@s.whatsapp.net', fee)
		...
		case 'unpremium':
				...
				break
                case 'bann':
				if (!isAdmin) return reply('*Apenas O Dono*')
				bnnd = body.slice(5)
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @558881060472 20`)
		...
		case 'nulis':
				...
				client.sendMessage(from, ct, image, {caption: '*Prontinho😊*', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis3':
				...
				client.sendMessage(from, gools5, image, {caption: '*Prontinho😊*', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis2':
		...
		case 'bplogo':
				...
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: '*Prontinho😊*', quoted: mek})
				await limitAdd(sender)
				break
		case 'hartatata':
				// By Diego⛔
				...
				client.sendMessage(from, bh, image, {caption: '*Prontinho😊*', quoted: mek})
				await limitAdd(sender)
				break
		case 'tahta':
		...
		case 'darkneon':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Desculpe, você foi pego!')
                    if (args.length == 0) return reply(`Uso: ${prefix + command} text\nExemplo: ${prefix + command} yogi`)
                    txt = args.join(" ")
                    reply('⌛ *AGUARDE POR FAVOR* ⌛')
                    buffer = await getBuffer(`https://videfikri.com/api/textmaker/${command}/?text=${txt}`)
                    client.sendMessage(from, buffer, image, {caption: 'Prontinho 😊.. *Segue Meu Criador La no Instagram @Rdiegosilvaa*', quoted: mek})
         break
         case 'gplaybutton':
                case 'splaybutton':
                if (!isRegistered) return reply( ind.noregis())
...
//dappa gay
	...
	break
		...
		case 'text2gif':
				...
				break
		case 'attp':
				if (args.length < 1) return reply(`_Cade O Texto Jovem?_\n*Exemplo ${prefix}attp Panda*`)
				attp2 = await getBuffer(`https://api.xteam.xyz/attp?file&text=${body.slice(6)}`)
				client.sendMessage(from, attp2, sticker, {quoted: mek})
				break
		case 'ttp':
				if (args.length < 1) return reply(`_Cade O Texto Jovem?_\n*Exemplo ${prefix}ttp Panda*`)
		...
		case 'randomkpop':
				...
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Prontinho kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
				...
				client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Prontinho aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep': 
				gatauda = body.slice(9)
				...
				client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Prontinho hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep2': 
				gatauda = body.slice(10)
		...
		case 'baixarmusica':
				...
				if (anu.error) return reply(anu.error)
				infomp3 = `*Canção  Encontrada!*\nTitulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.Tamanho}\n\n*ESPERE ENVIANDO POR FAVOR, SEM SPAM*`
		...
		case 'vinta':
				...
				client.sendMessage(from, vintage, image, {caption: 'Prontinho 😊 ${vin}', quoted: mek})
				await limitAdd(sender)
				break
                case '1cak':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Prontinho 😊 udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
		...
		case 'novafoto':
					if (!isOwner) return reply(ind.ownerb())
					client.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return reply(`Envie fotos com a legenda ${prefix}setbotpp ou tag de imagem que foi enviada`)
					enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(enmedia)
					await client.updateProfilePicture(botNumber, media)
					reply('Visual Novo Hehe 😗')
					break 
		case 'setppbott':
				if (!isAdmin) return reply('*Apenas O Dono*')
					client.updatePresence(from, Presence.composing) 
		...
		case 'pinterest':
           ...
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`Exemplo: ${prefix}glitchtext Panda`)
					reply(ind.wait())
	...
	if (!isRegistered) return reply( ind.noregis())
	  ...
	  teks = `_TITLE_:${anu.result.judul}\n\n_Aguarde o vídeo Está sendo enviado 😊`
	  client.sendMessage(from, buf, image, { quoted: mek, caption: teks })
	  client.sendMessage(from, bufq, video, { caption: 'Prontinho' }) 
	await limitAdd(sender)
	 break 
			case 'ytmp3':
					if (!isRegistered) return reply( ind.noregis())
					...
					client.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.result.judul}.mp3`, quoted: mek, caption: 'Prontinho'})
					await limitAdd(sender) 
					break 
			 case 'nsfwloli':
				    try {
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Esta é a lista de grupos Bot Panda :\n\n\`\`\``
					no = 0
		...
		case 'daftar':
                			...
                			if (namaUser.length >= 30) return reply(`Nome Muito Grande`)
...
╔═〔 𝙱𝙾𝚃 𝙿𝙰𝙽𝙳𝙰 〕
╠════➤ 𝘊𝘳𝘪𝘢𝘥𝘰𝘳: 𝙳𝙸𝙴𝙶𝙾 𝚂𝙸𝙻𝚅𝙰
╠════➤ 𝘐𝘯𝘴𝘵𝘢𝘨𝘳𝘢𝘮: @𝚛𝚍𝚒𝚎𝚐𝚘𝚜𝚒𝚕𝚟𝚊𝚊
╠════➤ 𝘕𝘶𝘮𝘦𝘳𝘰: wa.me/+558881060472 
╠════➤ 𝘠𝘰𝘶𝘵𝘶𝘣𝘦: https://www.youtube.com/channel/UCdQ2C52wbVqUjjgYgQzvmlg
╚══════════════════
╔═〔 𝚂𝚄𝙰 𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝙲𝙰𝙾 〕
╠════➤ 𝘗𝘳𝘦𝘧𝘪𝘹 : 『 ${prefix} 』
╠════➤ 𝘕𝘰𝘮𝘦 : ${pushname}
╠════➤ 𝘕𝘶𝘮𝘦𝘳𝘰 : wa.me/${sender.split("@")[0]}
╚══════════════════
...
╠═══➣ informações do bot
╠════➤ *${prefix}snk*
╠═══➣ Termos e Condições do Panda
╠════➤ ${prefix}covidindo
╠═══➣ informações Do Covid-19
╠════➤ *${prefix}creator*
╠═══➣ Dono
╚══════════════════
...
` 
		...
		case 'crosslogo':
				...
				if (isBanned) return reply('Desculpe, você foi pego!')
                    ...
                    client.sendMessage(from, tod, image, {caption: 'Prontinho 😊.. *Segue Meu Criador La no Instagram @Rdiegosilvaa*', quoted: mek})
         break
case 'bneon':
                case 'matrix':
                case 'breakwall':
		...
		case 'info':
					...
					teks = `*PROPRIETÁRIO DE INFORMAÇÕES*\n*Proprietário do bot* : Diegoツ\n*Numero Do Proprietário* : wa.me/558881060472\n*Proprietário* : www.instagram.com/rdiegosilvaa\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Nome do bot* : ${me.name}\n*Número do bot:* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Total De Bloqueados* : ${blocked.length}\n*O bot Está Ativo A* : ${kyun(uptime)}\n*Digite*: ${prefix}owner para entrar em contato com nosso administrador de bot.`
		...
		case 'snk':
					me = client.user
					uptime = process.uptime()
					teks = `*BOT PANDA* N1. Termos e Condições. Salvaremos seu texto e nome de usuário do WhatsApp no ​​servidor enquanto o bot estiver ativo. \ n2. Seus dados serão excluídos quando o bot estiver offline. \ N3. Não armazenamos imagens, vídeos, arquivos, áudios e documentos que você envia. \ N4. Nunca pedimos que você forneça informações pessoais. \ N5. Se você encontrar um bug / erro, informe diretamente ao proprietário do bot. \ N6. Apenas comande 1x se o bot não responder, por favor, repita novamente, se você repetir, não responde, o bot não está ativo \ n7. Não envie spam, compartilhe vírus virtex, telefone, videochamada, vamos bloquear você. OBRIGADO!`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
		case 'lpr':
					...
					client.sendMessage('558881060472@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'request':
					...
					break
		case 'blocklist': 
					teks = '*Esta é a lista de números bloqueados: Total* :\n'
		...
		case 'ocr': 
				...
				if (isBanned) return reply('Desculpe, você foi pego!')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`*Envie fotos com legendas ${prefix}ocr*`)
					}
					await limitAdd(sender)
					break
			case 'stickergif':
		...
		case 'tts':
				...
				if (args.length < 1) return client.sendMessage(from, 'O código do idioma é obrigatório!!', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Para qual texto você deseja dar voz?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
		...
		case 'gelud':
					...
					break
		case 'bot':
					client.sendMessage(from, 'BOT PANDA"',MessageType.text, { quoted: mek} )
					break
		case 'hargaprem':
		case 'hargapremium':
					...
					break
		case 'buypremium':
					client.sendMessage(from, 'Quer comprar premium? \ Nentre em contato conosco : wa.me/558881060472',MessageType.text, { quoted: mek} )
					break
		case 'buypremunlimited':
					
					client.sendMessage(from, 'Parabéns, você se inscreveu para sempre 🎉',MessageType.text, { quoted: mek} )
					break
		case 'buyprem30day':
					
					client.sendMessage(from, 'Parabéns pela sua assinatura de 30 dias',MessageType.text, { quoted: mek} )
					break
		case 'buyprem7day':
					
					client.sendMessage(from, 'Parabéns, você se inscreveu por 7 dias',MessageType.text, { quoted: mek} )
					break
		case 'test':
		...
		case 'unblock':
					...
					client.sendMessage(from, `*Pedido recebido, desbloqueando* ${body.slice(9)}@c.us`, text)
					break
		case 'leave':
					// By Diego⛔
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 AVISO BOT PANDA 」*\n\n${body.slice(4)}`)
						}
						reply('*Transmissão de sucesso* ')
					}
					break
		...
		case 'mudarfoto':
					...
					reply('*Alteração da Foto do grupo Foi Um Sucesso*')
					break				
		case 'add':
		case 'adicionar':
					if (!isGroup) return reply(ind.groupo())
		...
		case 'grupo':
            ...
            case 'creator':
            case 'developer':
                  client.sendMessage(from, {displayname: "Diego", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Este é o número do meu proprietário> _ <, não envie spam ou bloquearei você',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
           case 'mudarnome':
					if (!isGroup) return reply(ind.groupo())
                ...
                case 'setdesc':
					...
					client.sendMessage(from, 'Sucesso, Na Mudança Da descrição do grupo', text, {quoted: mek})
					break
           case 'demote':
           case 'rebaixar':
					if (!isGroup) return reply(ind.groupo())
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `*Comando aceito, rebaixado o administrador do grupo* :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`*Pedidos aceitos, baixados* @${mentioned[0].split('@')[0]} *Você Não e Mais Administrador Do Grupo*`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
			case 'promote':
		...
		case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Chutado Do Grupo* 🤭 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`*Chutado Do Grupo* @${mentioned[0].split('@')[0]} 🤭`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
		case 'listadmin':
		...
		case 'converter':
				// By Diego⛔
                 if (!isRegistered) return reply( ind.noregis())
				if (!isQuotedSticker) return reply('*Marcar a Figurinha Com #toimg Ou #converter*')
					reply(ind.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Sua Conversão Foi um SUCESSO* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                 case 'simi':
					if (args.length < 1) return reply('Cade O Texto Jovem?')
					teks = body.slice(5)
		...
		case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sucesso habilitado o modo nsfw neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Desativado o modo nsfw com sucesso neste grupo ✔️')
					} else {
						reply(ind.satukos())
					}
					break
		...
		case 'bemvindo':
					...
					if (args.length < 1) return reply('Ative pressione 1, Desative pressione 0')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*Ja Foi Ativo*')
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ Ativado o Recurso de boas-vindas neste grupo✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ Desativado o Recurso de boas-vindas neste grupo✔️')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ Ativado com Sucesso Recurso anti-link Neste Grupo ✔️')
						...
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						reply('❬ SUCESSO ❭ Desativado com Sucesso o Grupo anti-link Neste Grupo ✔️')
					} else {
                 ...
                 case 'event':
					if (!isGroup) return reply(ind.groupo())
					if (!isAdmin) return reply('*Apenas Adminban e Proprietário !*')
		...
		case 'clone':
					...
					if (args.length < 1) return reply(' *TAG QUE QUER NO CLONE !!!* ')
        ...
        if (budy.includes(`Panda`)) {

                  reply(`Sim? \ Não use o menu / ajuda do tipo de bot`)
       ...
       if (budy.includes(`@558881060472`)) {

                  reply(`Ele e Meu Criador Ele e Gostoso Ne?`)

                  }
                 
       if (budy.includes(`@558881060472`)) {

                  reply(`Não marque ele está ocupado de novo `)
