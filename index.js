...
case 'bneon':
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST BOT PANDA 」*\n\n${body.slice(4)}`)
						}
						reply('*Transmissão de sucesso* ')
					}
					break
		...
		case 'group':
           ...
           case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`*Pedidos aceitos, baixados* @${mentioned[0].split('@')[0]} *seja um administrador de grupo*`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
			case 'promote':
		...
		case 'welcome':
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						reply('Desativando com sucesso o grupo anti-link neste grupo ✔️')
					} else {
       ...
       if (budy.includes(`@558881060472`)) {
                  ...
                  reply(`Sim, existe uma tag PANDA 🗣`)
